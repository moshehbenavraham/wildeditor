-- Luminari Wilderness Editor Database Setup - PRODUCTION
-- 
-- MySQL Database Schema for Production Environment
...
-- Create database user for the wildeditor backend - PRODUCTION
-- Run these commands separately as a MySQL admin user:
-- CREATE USER 'wildeditor_prod_user'@'%' IDENTIFIED BY 'secure_production_password';
-- GRANT ALL PRIVILEGES ON luminari_mudprod.* TO 'wildeditor_prod_user'@'%';
-- FLUSH PRIVILEGES;

-- Verify spatial functions are working
SELECT 
  'Production spatial functions test' as test_name,
